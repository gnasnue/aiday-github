...
- **홈 상세 리포트를 히어로 카드 안 "자세히" CTA로 흡수** (`app/(main)/home/page.tsx`, `components/HeroDecisionBrief.tsx`, `components/ReportFeedback.tsx`) — 히어로 안의 "자세한 리포트" 텍스트 토글 → 히어로 아래 64px 펼침 카드를 거쳐, **근거 문장 끝 오른쪽의 `자세히 ⌄` CTA**(독립 행, 터치 타깃 44px)로 정착했다. 진입 카드를 따로 두면 표면이 둘이 되어 펼쳤을 때 "카드가 하나 더 생긴" 것처럼 보였다 — 이제 본문·잠정본 안내·trust line이 같은 카드 안 헤어라인 아래로 펼쳐지고, 화면의 L1/L2 표면이 하나 줄었다. 펼침 본문은 CTA 바로 아래에 오고 **근거 칩은 그 뒤에 남아** 접힘·펼침 어느 상태에서도 "칩이 마지막"이라는 순서(판단은 끝났고 근거는 확인용)가 유지된다. 상세 본문에서는 히어로 근거 문장으로 이미 쓴 줄을 제외한다 — 같은 카드 안에서 같은 문장이 네 줄 아래 또 나오면 복사해 붙인 글처럼 읽힌다. 종전 진입 행이 상시 노출했던 판단 기준 보조문("○○ 체질 · 등원 09:00 기준")은 이 정리로 사라졌고, 같은 정보는 펼친 뒤 trust line이 담당한다. 피드백 행의 자체 divider·`px-0.5`를 제거해 카드가 divider를 소유하게 했다(좌측선 x=40 정렬, 선 두 겹 제거). 로딩 스켈레톤도 새 골격(기준값 블록·CTA 행)으로 맞춰 로딩→실물 전환의 높이 점프를 100px → 16px 이내로 줄였다.
