...
- **홈 상세 리포트 진입을 펼침 행으로** (`app/(main)/home/page.tsx`, `components/ReportFeedback.tsx`) — 히어로 안의 "자세한 리포트" 텍스트 토글을 히어로 바로 아래 64px 펼침 행으로 승격했다. 보조문에 **판단 기준**("○○ 체질 · 등원 09:00 기준")을 상시 노출한다 — 종전 trust line은 펼쳤을 때만 렌더돼 대부분의 사용자가 "무엇을 근거로 판단했는지"를 한 번도 보지 못했다. 피드백 행의 자체 divider·`px-0.5`를 제거해 카드가 divider를 소유하게 했다(좌측선 x=40 정렬, 선 두 겹 제거).
