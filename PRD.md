...
| **팀명** | 아이데이 (AiDay) |
| **작성일** | 2026-07-08 |
| **문서 버전** | v2.3 — 변경 이력은 [버전 히스토리](#버전-히스토리) 참조 |
...
| **FGI** | Focus Group Interview. 만 1~8세 자녀를 둔 부모 대상으로 수행한 자체 유저 인터뷰 ([섹션 03](#03-현재-어떤-문제가-있는가) 참조) |
...
**비즈니스 가치:** 이 판단은 매일 아침 반복되므로, 성공하면 데일리 리텐션이 구조적으로 내장된 서비스가 된다. 사용할수록 아이 프로필과 수용/거부 이력(Family Memory 원료)이 쌓여 "내 아이를 아는 서비스"라는 전환 장벽이 형성된다 — 일반 날씨 앱이나 생성형 AI가 복제하기 어려운 개인화 해자다. 수익화(프리미엄 케어 리포트, 육아용품 제휴 등)는 공개 베타에서 "매일 쓰는 서비스인가"(아침 재방문율, [섹션 11](#11-성과-측정-방법-kpi))를 검증한 뒤 결정한다. FGI 기준 사용 의향은 89%로 높으나 지불 의사는 미검증이며, 본 PRD 범위는 무료 운영으로 그 전제를 검증하는 단계다.
...
- **자체 FGI(만 1~8세 자녀 부모 대상 유저 인터뷰):** **76.4%**가 아침 등원 준비 시 옷차림 결정에 어려움을 겪고 있으며, 미세먼지로 인한 야외활동 낭패 경험도 **76.4%**. **88.2%**는 매일 환경 정보를 탐색하면서도 "파편화된 정보를 아이 기준으로 해석하는 과정 자체"를 최대 부담으로 꼽았고, 옷차림 결정이 어려운 상황 1위는 **"일교차가 큰 환절기"(82.4%)**. 향후 사용 의향 **89%**, 최고 수요 기능은 당일 아침 자동 알림(**76.5%**)
- FGI는 소규모 자체 조사로, 통계적 대표성 확보보다 문제의 실재와 우선순위를 정성적으로 확인하는 목적으로 수행했다. 정량 검증은 공개 베타의 실사용 지표([섹션 11](#11-성과-측정-방법-kpi))로 대체한다
...
> [!NOTE]
> 대표 페르소나 "김서연"은 FGI 응답 패턴(옷차림 결정 어려움 76.4%, 일교차 어려움 82.4%, 해석 부담 88.2%)과 실사용 시나리오를 결합해 구성한 가상 인물이며, **판단 엔진의 표준 테스트 케이스**([섹션 05](#05-문제-해결을-위한-기능)·[11](#11-성과-측정-방법-kpi))로도 사용한다.
...
| **사용 의향** | FGI 기준 89% — 최고 수요 기능은 "당일 아침 자동 알림"(76.5%) |
...
| 2 | 안전·비용 보호 레이어 | 규칙 기반 안전 판정 선행, 표현 안전 필터, 레이트리밋(게스트·로그인), Fallback Chain |
...
- **`/api/report` 레이트리밋:** 비로그인(게스트) IP당 일 10회 — 체험 한도. 로그인 사용자 계정당 일 30회 — 아이 여러 명 × 아침·저녁 재조회를 감안한 실사용 여유 한도이자 폭주 계정의 비용 상한. 초과 시 429 + 안내 + 규칙 기반 기본 추천으로 전환
...
| **내용 규칙** | 제목은 브리핑 훅 형식(한 줄 결론, 25자 이내), 본문은 당일 예보×일정 교차 기반 핵심 액션 1~2개. 건강 결과 약속 표현 금지(표현 안전 필터 적용). 전날 밤 준비 알림·카카오톡 발송도 동일 형식(전날 밤은 익일 예보 기반 준비물 중심) |
...
- LLM 비용 월 ≤ 5만원 — 레이트리밋(게스트 일 10회·로그인 계정당 일 30회) + 클라이언트 5분 캐시로 통제
...
| Phase | 기간 | 목표 | 완료 조건 |
|-------|------|------|-----------|
| Phase 1 | 1주차 | 안전·비용 보호 레이어 | 게스트 11번째·로그인 31번째 호출 429 + 안내 표시 / 경보 규칙 판정이 LLM보다 선행 / 표현 필터 작동 / API 장애 시 fallback 3초 내 표시 |
...
- 제품 KPI 목표치 판정은 PostHog 연동(Phase 6) 이후 개시 — 수치 없는 감 판정 금지.
