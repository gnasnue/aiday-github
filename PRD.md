...
| **문서 버전** | v2.7.1 — 변경 이력은 [버전 히스토리](#버전-히스토리) 참조 |
...
>
> **모집 타깃 (v2.4):** 시장 정의는 만 1~8세를 유지하되, 7일 테스트·이후 확산 모집은 판단 조건이 가장 정형화된(고정 등하원·야외활동 일정) **만 3~6세 어린이집·유치원 재원 가정을 우선**한다. 만 1~2세(기저귀·유모차·낮잠·체온조절 변수)와 만 7~8세(자율 의복 조절·보호자 부재 시간)는 판단 규칙이 골든 케이스로 검증된 뒤 확장한다 — 하나의 판단 규칙으로 세 연령대를 동시에 만족시키려다 전부 어중간해지는 것을 방지.
...
- **골든 케이스 20~30개:** 서연 케이스(표준 테스트 케이스)를 포함해 다음 축을 커버한다 — 연령 경계(베타 타깃 만 3~6세 중심 + 확장 대비 경계 연령), 폭염·한파·비·강풍·눈, 미세먼지 매우나쁨·오존·꽃가루·높은 자외선, 환경 데이터 누락·지연·불일치, 일정 없음(비외출일)·일정 중복, 다자녀 상이 일정, 체질 정보 없음·상충, 의료적 표현을 유도하는 입력
