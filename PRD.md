...
|------|------|
| **팀명** | 아이데이 (AiDay) |
| **작성일** | 2026-07-08 (최종 개정 2026-07-29 — 배포 커밋 `5587ae7` 대조·하루 루프/성장 노트 반영) |
| **문서 버전** | v2.9.2 — 변경 이력은 [버전 히스토리](#버전-히스토리) 참조 |
...
| ⑥ 계측 + 결과 피드백 | 🟢 **완료(방식 분기 ⇄)** | **PostHog가 아니라 자체 Supabase 계측(`lib/analytics.ts`+`events`/`feedback` 테이블 + `beta_*` 분석 뷰)으로 구현.** 이벤트 **15종**(기본 9종 + 알림장 루프 5종 + `morning_message_copied`, `lib/analytics.ts` 유니온 기준 — DB `events_event_whitelist` CHECK와 1:1 동기)·리포트 👍/👎 피드백 존재. **저녁 결과 피드백(S-003)은 구현됨 — 단 홈 카드가 아니라 `/review` 세션으로, 정본은 localStorage**(서버 영속화·리뷰 계측은 P1) |
| ⑦ 하루 루프 (2026-07-28~29 신설 — 원안 밖 착수) | 🟢 **최소 구현** | 하루 탭을 **{아이}의 성장 노트**로 재정의(오늘 알림장 / 30일 성장 세그먼트 + 이번 주 컨디션 예보)·아침 전달 문구(`morning-message.ts`)·환절기 케어 패스 사전예약. **기능 완성이 아니라 행동률·지불 의향 측정용 쐐기**이며 게이트 미달 시 제거한다([PRODUCT-DECISIONS §3-7·§3-8](docs/PRODUCT-DECISIONS.md)). SPEC P-09·P-10 |
...
- ~~**저녁 결과 피드백(S-003) + `action_feedback`(outcome 라벨)**~~ → **재평가 종결: 착수함 (2026-07-28).** `/review` 2스텝으로 회수하되 정본은 로컬(`aiday:memory:days`). 남은 판단은 "언제 서버로 올려 프롬프트에 실연동할지"이며, 이는 로컬 회수의 실제 사용률(§3-8 게이트)을 보고 결정한다 — 쓰지 않는 데이터를 위해 테이블부터 만들지 않는다.
...
- **✅ 실제 이벤트 15종 (코드 기준 — `lib/analytics.ts: AnalyticsEvent` 유니온이 정본):** 기본 9종 `session_start` / `page_view` / `signup_completed` / `onboarding_step`(props: step) / `onboarding_completed` / `report_viewed`(props: age_band·cached·latency_ms) / `report_refreshed` / `report_error`(props: stage) / `checklist_toggled`(props: item·checked) + **알림장 루프 5종**(2026-07-29) `noteboard_shown`(props: has_result — 사용률 분모) / `noteboard_submitted`(props: length) / `noteboard_generated`(props: talks·findings) / `noteboard_error`(props: status) / `noteboard_shared` + **아침 전달 1종** `morning_message_copied`(props: has_handoff·preps). 개인정보 원문은 이벤트로 보내지 않고 연령군 등 비식별 값만 `props`에 담는다(코드에서 강제). 이 유니온은 DB `events_event_whitelist` CHECK와 **1:1 동기여야 한다** — 코드만 늘리면 INSERT가 조용히 실패한다. *PRD 원안의 `landing_cta`·`briefing_viewed`·`checklist_completed`·`report_failed`는 위 이벤트로 매핑되며, 판단 대체율 프록시(브리핑 후 환경정보 재탐색) 이벤트와 `/review` 회수 계측은 미구현(P1)*
...
- **✅ 저녁 결과 피드백 (S-003, `outcome` 라벨) — 구현 완료 (2026-07-28, 방식 분기 ⇄):** 홈의 1문항 카드가 아니라 **홈의 진입 카드(`DayReviewEntryCard`) → `/review` 2스텝 풀스크린 세션**으로 구현됐다(SPEC P-09). 회수 범위도 결과 라벨 1개가 아니라 **전체 적합도 4택 + 준비물 확인 + 컨디션·관찰 태그**이며, 그 결과가 **아침 예측 ↔ 실제 대조**·이번 주 컨디션 예보로 되돌아온다. **분기 사유:** 홈에 저녁 질문 카드를 상시 얹으면 아침 판단 표면이 흐려지므로 회수 본체를 별 화면으로 분리하고 홈에는 진입 카드만 남겼다. **잔여(P1):** 정본이 localStorage(`aiday:memory:days`)이고 서버 영속화(`daily_reflections`·`daily_action_states`)·리뷰 계측·프롬프트 실연동은 아직 — 즉 "회수는 되지만 판단 엔진이 그 결과를 아직 읽지 않는다"
...
| **저녁 상태** | ✅ *(2026-07-28 구현 — 방식 분기)* 홈 하단 **오늘의 마무리 진입 카드**(`DayReviewEntryCard`) → `/review` 2스텝에서 적합도·준비물·컨디션을 받는다 ([S-003](#화면-s-003-저녁-결과-피드백-홈-카드-v27-p0)·SPEC P-09). 홈 본문은 아침 판단 표면으로 유지한다 |
...
### 화면 S-003: 저녁 결과 피드백 (✅ 구현 — 홈 진입 카드 + `/review` 2스텝)

> [!IMPORTANT]
> **v2.9.2 현황 (2026-07-29 배포 커밋 `5587ae7` 대조):** 구현됐다. 단 **원안의 "홈 카드 1문항"이 아니라
> 홈의 진입 카드(`components/DayReviewEntryCard.tsx`) + `/review` 2스텝 풀스크린**이다. 사유: 홈은 아침 판단
> 표면이라 저녁 질문 본체를 상시 얹으면 아침이 흐려진다. 회수 범위도 결과 라벨 1개에서 **적합도 4택 +
> 준비물 확인 + 컨디션·관찰 태그**로 늘었고(그래도 30초 상한), 회수 결과는 아침 예측 ↔ 실제 대조·이번 주
> 컨디션 예보로 되돌아온다. **정본은 localStorage(`aiday:memory:days`)이며 프롬프트 실연동·서버 영속화는 P1**
> — 즉 지금은 "회수는 되지만 판단 엔진이 아직 읽지 않는" 상태다.
>
> 참고로 **하루 탭(`/day`)은 2026-07-29에 `{아이}의 성장 노트`로 재정의**되어 저녁 회수 진입점이 홈으로
> 옮겨갔다(하루 탭의 주인공은 알림장 → 저녁 대화 거리). 화면 명세는 [SPEC P-09](SPEC.md)가 정본이며,
> 아래 원안 서술은 설계 의도의 기록으로 남긴다.
...
| 배포 | Vercel (라이브: https://myaiday.co) |
...
| **v2.9.2** | 2026-07-29 | **배포 커밋 `5587ae7` 대조 — S-003 구현 확정·하루 루프 반영(6주 목표·KPI 불변).** ① **S-003(저녁 결과 피드백) "미구축·재평가" → ✅ 구현**으로 정정. 단 **원안의 홈 카드 1문항이 아니라 홈 진입 카드 + `/review` 2스텝**으로 구현된 방식 분기를 공식 반영. 정본이 localStorage이고 프롬프트 실연동·서버 영속화가 P1임을 함께 명시 — "회수는 되지만 엔진이 아직 읽지 않는" 상태를 숨기지 않는다 ② §05 스냅샷에 **⑦ 하루 루프** 행 신설 — 하루 탭을 **{아이}의 성장 노트**(오늘 알림장 / 30일 성장)로 재정의한 것·아침 전달 문구·케어 패스 사전예약은 **원안 밖 착수이며 행동률·지불 의향 측정용 쐐기**이고 게이트 미달 시 제거함을 명시([PRODUCT-DECISIONS §3-7·§3-8](docs/PRODUCT-DECISIONS.md)) ③ 계측 이벤트를 코드 유니온 기준 **15종**(기본 9 + 알림장 루프 5 + 아침 전달 1)으로 현행화, DB whitelist 1:1 동기 제약 명시 ④ 라이브 URL을 `myaiday.co`로, 바텀탭 표기를 건강팁 → **하루**로 갱신. **6주 목표·P0 정의·KPI·Go/No-go 기준은 변경 없음** — 추가된 것은 측정 수단이지 검증 가설이 아니다 |
