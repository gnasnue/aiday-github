...
- **홈 "추천 아이템" 섹션 유지 (2026-07-12 결정 — 숨김 방침 철회):** 더미 카드(일러스트·가격 표시) 형태로 홈에 유지한다. 단, 실제 쇼핑·커머스 연동은 계속 Non-Goals
...
- 쇼핑·커머스 연동 — 사전 설문에서도 기능 선호 최하위(5.9%)·유용성 평균 2.8/5로 수요 최저. 홈 추천 아이템 카드 자체는 더미 상태로 유지하되(2026-07-12 결정) 실제 구매 연결은 만들지 않는다
...
+------------------------------------+
| 지우를 위한 오늘의 추천 아이템      |   ← 더미 카드 유지 (커머스 연동 없음)
+------------------------------------+
| 홈 | 환경정보 | 옷차림 | 건강팁 | 마이 |
+------------------------------------+
```
