...
| **팀명** | 아이데이 (AiDay) |
| **작성일** | 2026-07-08 |
| **문서 버전** | v2.7 — 변경 이력은 [버전 히스토리](#버전-히스토리) 참조 |
...
- **브리핑 단일 원본 (`daily_briefings`):** 브리핑은 아이당 하루 1회 생성·저장하고, 홈과 (7일 테스트 동안) 수동 발송 메시지가 **같은 레코드**를 재사용한다 — 발송 메시지와 홈 화면의 내용 불일치("왜 아까와 다르지?") 방지, LLM 비용 통제, 문제 발생 시 재현성(입력 스냅샷·프롬프트/모델 버전 기록) 확보. 예보가 임계값 이상 변한 경우에만 새 버전을 생성하고 홈에 변경 사유를 표시한다. 사용자 수동 재생성은 소수 회로 제한
...
- **지역 선택·데이터 정합 (v2.7 — 엔지니어링 리뷰 T2):** 현재 홈은 화면에 강남구를 표시하면서 실제로는 서울시청 좌표·종로구 측정소를 고정 호출한다 — 브리핑 근거가 되는 데이터가 잘못 표시되는 출시 블로커다. **온보딩에 지역 선택 1개를 추가하고, 표시명·기상 좌표·대기질 측정소를 단일 매핑 테이블에서 함께 관리**한다. 자동 위치 권한은 후순위. (디자인 리뷰: 지역·데이터 기준 시각·실패 상태는 숨기지 말고 홈에 짧고 명확하게 표시)
...
- **선행 규칙 레이어:** 절대 안전 기준(경보 임계값)은 `lib/safety-rules.ts`에서 LLM 호출 전 판정, 결과를 체크리스트에 강제 병합. (로드맵 3·4단계 착수 시 시뮬레이션 점수·최적화 추천도 이 규칙 코드가 담당하고 LLM은 문장화만 맡는다 — 화면 비교값과 추천의 불일치 방지)
...
기존 `children.notif`(json)를 발송 설정 원본으로 사용(온보딩 Step 7에서 이미 수집 중), `children.schedule`(json)을 판단 입력 축 ③으로 사용. 지역 선택(S-002)은 표시명·기상 좌표·대기질 측정소를 한 매핑으로 묶어 프로필에 저장.
...
- `npm run build` · `npm run lint` 통과 + **골든 케이스 전체 통과(치명적 안전 오류 0건)**가 판단 엔진 관련 PR의 머지 조건. 다음 변경부터 API 경계·핵심 판단 규칙에 최소 자동 테스트를 추가한다(엔지니어링 리뷰)
...
| Phase | 기간 | 목표 | 완료 조건 |
|-------|------|------|-----------|
| Phase 1 | 1주차 | 안전·비용 보호(T1) + 최소 계측(T5) | 게스트 11번째·로그인 31번째 호출 429 + 안내 / 경보 규칙 판정이 LLM보다 선행 / 표현 필터 작동 / API 장애 시 fallback 3초 내 표시 / 최소 이벤트 6종(`landing_cta`·`onboarding_completed`·`briefing_viewed`·`checklist_completed`·`report_failed`·`evening_feedback_submitted`) 기록 |
| Phase 2 | 2주차 | 신뢰성 정합 — 지역·데이터·프로필·동의·삭제 | **지역 선택(S-002) + 표시·좌표·측정소 정합(T2)** / **새벽 기상 경계 처리(T3)** / 온보딩 실프로필로 마스크·보습 추천 작동 / `aiday:` 통일 / 예시 뱃지·추천 아이템 숨김 / 약관·처리방침 실문서(국외 처리 고지) + **민감정보 별도 동의** + 탈퇴 연쇄 삭제(T4) |
| **Phase 3** | 3주차 | **판단 엔진 + 브리핑 단일 원본 (핵심)** | 일정 시각별 예보 교차 평가 / 비외출일 실내 케어 브리핑 / 홈 시간대별 카드 hourlyForecast 실측값 / `daily_briefings` 저장·재사용(홈=발송 메시지 동일) / `action_feedback` 체크·저녁 피드백(S-003) 저장 / **골든 케이스 1차(표준 케이스 포함 10개 내외) 통과** |
| Phase 4 | 4주차 | **7일 컨시어지 테스트 준비·개시** | 참여 가구 모집(지인 15~20) / 수동 발송 절차·기록(`notifications_log`) 정비 / 매일 5개 관찰 항목 기록 시작 / 골든 케이스를 20개로 확장 |
| Phase 5 | 5주차 | **테스트 운영 + 판단 품질 완성** | 7일 관찰 데이터 수집 / 컨시어지 피드백을 few-shot·골든 케이스 기대값에 반영 / **골든 케이스 30개 완성 + 자동 평가 머지 게이트 적용** |
...
**7일 컨시어지 테스트 상세:** 지인 15~20가구 대상. 매일 아침 브리핑을 **사람이 수동 생성·발송**하고 다섯 가지만 기록한다 — ① 아침에 확인했는가 ② 어떤 추천을 실행했는가 ③ 다른 앱으로 재확인했는가 ④ 저녁 결과(더움/적당/추움) ⑤ 내일도 받고 싶은가. 결과는 골든 케이스의 실제 입력·기대 출력과 few-shot 소스로 쓰고, 반복 사용 가설을 **인터뷰(사용 의향)가 아니라 행동 관찰**로 검증한다. 이 테스트가 자동 알림·시뮬레이션·최적화 착수의 판단 근거다.
...
- [ ] `action_feedback` 마이그레이션 + 체크/해제·저녁 피드백(S-003) 저장 연결
...
6주차 말, 7일 컨시어지 테스트(15~20가구)·골든 케이스 결과·계측 데이터로 **다음 단계(자동 알림·시뮬레이션·최적화) 착수 여부**를 판정한다. 수치는 업계 표준이 아니라 초기 실험용 제안값이며, 미달 영역은 보강 후 재판정한다 — **순연은 실패가 아니다.**
