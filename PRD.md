...
| **팀명** | 아이데이 (AiDay) |
| **작성일** | 2026-07-08 |
| **문서 버전** | v2.6 — 변경 이력은 [버전 히스토리](#버전-히스토리) 참조 |
...
### AI 성숙도 로드맵 — 자동화 → 예측 → 시뮬레이션 → 최적화 (v2.6)

AI 도입의 가치는 자동화에서 멈추지 않고 **① 자동화 → ② 예측 → ③ 시뮬레이션 → ④ 최적화**의 순서로 성숙한다. 자동화는 데이터를 축적하는 시작 단계이고, 최종 가치는 축적된 데이터로 개인에게 최적화된 판단을 내리는 데 있다. 아이데이의 제품 구조를 이 4단계에 매핑하면 다음과 같다 — **본 PRD(6주)의 구현 범위는 1·2단계**이며, 3·4단계는 1·2단계가 수집하는 데이터 위에서 착수한다.

| 단계 | 아이데이에서의 구현 | 범위 |
|------|--------------------|------|
| **1단계 — 자동화** | 매일 아침 브리핑 **자동 생성·선제 발송**. 부모가 매일 반복하던 환경 정보 수집·해석·판단의 수작업을 제거하고, "부모가 묻고 AI가 답한다"를 "AI가 먼저 제안하고 부모가 승인한다"로 전환 | ✅ 본 PRD P0 ([판단 엔진](#p0--데일리-케어-브리핑-판단-엔진-핵심) · [선제 발송](#p0--선제-브리핑-발송-pwa--당일-아침-웹-푸시)) |
| **2단계 — 예측** | 시간대별 환경 궤적 × 아이 체질·일정을 교차해 **오늘 하루의 리스크를 선제 예측** — 예: 땀 많은 체질 × 일교차 11° → 젖은 옷 → 체온 급강하 리스크 사슬. 익일 예보 기반 준비물 예측(전날 밤 알림)으로 확장 | ✅ 본 PRD P0 (판단 엔진 CoT에 내재) / 전날 밤 알림은 [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널) |
| **3단계 — 시뮬레이션** | 케어 옵션별 결과를 비교하는 **what-if 시나리오**("이 옷차림이면 하원 때 어떨까"). 내부적으로는 골든 케이스 자동 평가가 이미 판단 엔진의 시뮬레이션(고정 입력 → 출력 검증) 역할을 수행 | ⏸ 사용자 대상 기능은 공개 베타 검증 후 — 내부 시뮬레이션([판단 품질 평가](#p0--판단-품질-평가-eval--v24에서-p1--p0-승격))은 본 PRD P0 |
| **4단계 — 최적화** | **Family Memory** — 수용/거부 로그와 저녁 결과 피드백(더움/적당/추움)이 누적되어, 추천이 "일반적인 만 4세"가 아니라 **"내 아이"에게 최적**으로 수렴. 데이터 우위와 전환 장벽의 원천 | ⏸ 장기 로드맵(OverEdge) — 단, 원료(`action_feedback` 피드백 로그) 수집은 본 PRD P0에서 시작 |
...
| 5 | 선제 브리핑 발송 | PWA + **당일 아침 웹 푸시 1종에 집중 (v2.6)** — 전날 밤 알림·카카오톡은 P0에서 제외, [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널)으로 분리 |
...
> **P0 채널은 당일 아침 웹 푸시 1종으로 한정한다 (v2.6).** 핵심 가설(아침 재방문율, [섹션 11](#11-성과-측정-방법-kpi))의 검증에는 아침 브리핑 1종이면 충분하다. 전날 밤 알림·카카오톡은 착수 조건과 함께 [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널)으로 분리한다 — "실제 구현할 것만 쓴다" 원칙의 적용이며, 6주 1인 개발 일정에서 판단 품질(제품의 본질)에 시간을 우선 배분하기 위함이다.
...
### 조건부 확장 — 전날 밤 알림과 카카오톡 채널

**P0가 아니다.** 아래 착수 게이트를 통과할 때만 구현하며, 통과하지 못하면 구현하지 않은 채로 공개 베타에 진입한다(베타 직후 fast-follow 후보). 이 항목들의 지연은 베타 공개를 막지 않는다.

| 항목 | 내용 | 착수 게이트 |
|------|------|------------|
| **전날 밤 준비 알림** | 설정 시각(기본 21:00 — 설문 희망 시간대 2위 '전날 밤 20~22시' 37.5%)에 익일 예보 기반 준비물 요약(여벌 옷·가방 챙기기 등)을 발송 — 준비물은 전날 밤에 챙기는 실사용 흐름 지원(사전 설문 기능 수요 2위 64.7%). 아침 브리핑과 동일한 Cron·이력 인프라 재사용(발송 type만 추가)이라 증분 비용이 작다 | Phase 4의 아침 발송 완료 조건이 조기 충족되어 잔여 시간이 있거나, Phase 5가 B안(판단 품질 개선)으로 분기한 경우 착수 |
| **카카오톡 "나에게 보내기" 채널** | 카카오 로그인(Supabase Kakao OAuth) + `talk_message` 권한으로 브리핑을 본인 카카오톡으로 발송. iOS 웹 푸시는 PWA 홈 화면 설치가 전제라 도달 공백이 있고, 사전 설문 선호 채널 1위도 카카오톡(52.9%, 앱 푸시 35.3%) — 다만 실제 병목 여부는 데이터로 확인한 뒤 착수한다. 착수 시 토큰 최초 캡처·갱신·폐기, 연결 해제, 탈퇴 시 토큰 완전 삭제까지가 완료 조건(소셜 로그인 연결만으로 토큰 운영이 자동 해결되지 않음 — Supabase는 프로바이더 토큰을 보관·갱신해 주지 않음) | **4주차 웹 푸시 퍼널 데이터로 판정** — iOS 도달(PWA 설치율·권한 허용률)이 실제 병목으로 확인될 때만 Phase 5 A안으로 착수 ([PRODUCT-DECISIONS §3-3](docs/PRODUCT-DECISIONS.md) 컨틴전시의 발동 기준 구체화) |

- **채널이 2개가 되는 시점의 원칙 — 주 채널 1개 + 폴백 (v2.4):** 사용자는 주 채널(웹 푸시 또는 카카오톡)을 하나 선택하고, 보조 채널은 주 채널 발송 실패·비활성 시에만 사용한다 — 아침·밤 × 푸시·카톡으로 하루 최대 4회를 받게 되는 알림 피로(구독 해제의 최대 원인) 방지. "둘 다 켜면 양쪽 수신" 설계는 금지
...
> [!NOTE]
> v2.6에서 구 "P1 — 있으면 좋음" 계층을 삭제했다. **실제 구현할 것(P0)과 구현하지 않을 것(Non-Goals) 두 가지만 남긴다** — "있으면 좋은 것" 목록은 범위 확장의 통로가 되므로 두지 않는다. 아래 일부 항목은 재검토 조건을 명시한다(조건 충족 전까지는 구현하지 않음).
...
- 카카오 알림톡(비즈메시지) — 발신프로필·템플릿 승인 리드타임이 6주 초과, 공개 베타 검증 후 재검토 ([조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널)의 카카오톡 "나에게 보내기"와 별개 채널)
- **래퍼앱 (WebView/Capacitor)** — 웹앱을 감싸 스토어에 배포하고 네이티브 푸시(FCM/APNs)를 확보해 iOS 웹 푸시의 "홈 화면 설치 필수" 제약을 근본 해소하는 다음 단계이나, 개발자 계정·스토어 심사 리드타임이 6주를 초과 — **공개 베타 이후 재검토** (앱 형태 단계화 결정: [docs/PRODUCT-DECISIONS.md](docs/PRODUCT-DECISIONS.md) §3-5)
- **변화 감지형 알림** (옷차림·준비물이 평소와 달라지는 날만 강하게 알리는 발송 모드) — 사전 설문 자유 응답에서 자발적으로 요청된 수요이나, "매일 브리핑 vs 변화 감지 브리핑" 습관 모델 실험은 **베타의 알림 피로 데이터(차단·구독 해제율) 확보 후 재검토**
- **Family Memory Engine**(수용/거부 이력의 판단 반영·개인화 학습 — [AI 성숙도 로드맵](#ai-성숙도-로드맵--자동화--예측--시뮬레이션--최적화-v26) 4단계) — OverEdge 장기 로드맵([용어 정의](#용어-정의) 참조) 항목. 단, 그 원료인 액션별 수용/거부 로그 수집은 이번 범위(P0 계측)에서 시작한다
- 네이티브 앱 재개발(React Native/Flutter 등) — 앱 형태는 단계적으로 전환한다: **PWA(본 PRD) → 래퍼앱(베타 이후) → 네이티브 재개발(공개 베타 검증 후 별도 결정)**
...
> [!NOTE]
> 아래 와이어프레임의 🌙 전날 밤 준비 알림·💬 카카오톡 연동 항목은 [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널) 착수 전까지 미노출하거나 "준비 중"으로 표기한다. **P0 구현 범위는 푸시 권한 획득 + 당일 아침 브리핑 설정까지다.**
...
| 카카오톡 채널 (조건부 확장 — 착수 게이트 통과 시) | 카카오 로그인(Supabase Kakao OAuth) + Kakao `talk_message` API |
| 제품 계측 (신규) | PostHog |
...
| Phase 4 | 4주차 | 선제 알림 인프라 (PWA + 당일 아침 웹 푸시) + 알림 퍼널 계측 | S-002에서 구독 성공 / 아침 알림 ±10분 내 발송 / OFF 사용자 미발송 / 중복 발송 없음(고유 제약) / 예보 실패 시 발송 스킵 / 설치·권한 퍼널 이벤트 기록 → **주말에 카카오 착수 게이트 판정** (조기 완료 시 전날 밤 알림 착수 가능 — [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널)) |
| Phase 5 | 5주차 | **게이트 분기** — iOS 도달 병목 확인 시 카카오톡 채널, 아니면 판단 품질 개선 | (A안) 카카오 연동 → 카톡 도달 / 토큰 캡처·갱신·폴백 작동 (B안) 컨시어지 피드백 반영 프롬프트 개선 + 전날 밤 준비 알림(아침 인프라 재사용) — **공통: 골든 케이스 20~30개 완성 + 자동 평가 머지 게이트 적용 + 저녁 결과 피드백 UI 작동** |
...
> [!WARNING]
> **일정 컨틴전시:** 6주에 P0 6블록은 여유가 없는 일정이다 — v2.6에서 알림 채널을 당일 아침 웹 푸시 1종으로 압축해 부담을 낮췄다. 전날 밤 알림·카카오 채널은 [조건부 확장](#조건부-확장--전날-밤-알림과-카카오톡-채널)이므로 게이트를 통과하지 못하면 구현하지 않은 채 베타에 진입한다(베타 직후 fast-follow 후보) — 이 항목들의 지연이 베타 공개를 막지는 않는다. 반대로 **골든 케이스·자동 평가가 미완성이면 공개 베타를 순연한다** — 판단 품질이 미검증인 채로 공개하는 것이 최대 리스크이며, 케이스 수를 줄여서 일정을 맞추지 않는다.
