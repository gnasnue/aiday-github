...
- **홈 히어로 근거 칩 (evidence chip) — 현행 계약 (2026-07-26 확정, 이 표가 단일 진실):**
  ...
  | 후보 ① 등급 지표 | 미세먼지·꽃가루·자외선·바람·건조 **+ 더위·추위**(2026-07-27 추가) — **warn 등급일 때만** 칩이 된다. "좋음·보통"은 칩을 만들지 않는다 |
  | 후보 ② 계량 지표 | 일교차·강수 — 등급이 아니라 수치·사실이므로 **등급 없이 뉴트럴 칩**. 이 부류가 있어야 무난한 날에도 칩이 뜬다. **일교차는 8°C↑면 warn으로 승격**(2026-07-27) |
  | 더위·추위 임계 | 더위 = 불쾌지수 **DI≥80** 또는 기온 **≥33°** → "매우 심함", **DI≥76** → "심함". 추위 = 기온 **≤0°** → "심함". **하나의 신호로 합쳐 센다**(폭염과 고온다습을 칩 2개로 세면 issueCount가 부풀고 칩 행이 더위로 다 찬다). 임계는 `lib/outdoor-index.ts` `decisiveDeterrent`에서 차용 — 새 숫자를 만들지 않는다. DI는 기온 24° 미만에서 계산하지 않아 **서늘하고 습한 날은 더위가 아니다** |
  | 금지 | **현재·체감 기온은 칩으로 만들지 않는다** — 우상단 기준값 블록과 중복. 일교차는 "기온의 폭"이라 중복이 아니다. 더위·추위도 **등급 표기만** 쓴다("더위 매우 심함" ⭕ / "더위 35°" ❌ — 수치는 기준값 블록의 몫) |
  ...
  **warn 어휘를 고칠 때는 `app/(main)/home/page.tsx slotNotables()`를 같은 커밋에서 함께 고친다.** 한쪽만 바뀌면 "히어로는 더위 경고 / 케어 플랜 특이사항은 무난해요"로 갈린다 — 2026-07-27에 실제로 그렇게 갈려 있었다.
...
| 2026-07-27 | **히어로 warn 어휘에 더위·추위 추가 + 일교차 8°C 승격 (2026-07-26 보류 항목 해소)** — 증상: 조건 pill이 **"낮 31도 고습"이라고 그날의 1순위 조건을 이름으로 말하는데 색은 뉴트럴**이었다(실사용 제보). 이 카드에서 뉴트럴은 **"색 없음이 곧 특이사항 없음"**으로 정의된 색이라(2026-07-26 결정 (4)) 카드가 자기모순이었다. 원인은 회귀가 아니라 **설계 공백**: `heroState`의 warn 어휘가 강수·미세먼지·꽃가루·자외선·바람·건조 6종뿐이고 **더위·고습이 처음부터 없었다**(`git log -S"폭염"`·`-S'"heat"'` 모두 이력 0건). 그래서 폭염·고온다습이 그날의 유일한 이슈인 날은 구조적으로 `issueCount = 0` → `normal`이 됐다. 앱의 나머지는 이미 이상 신호로 다루고 있었다 — 시간대별 환경 카드는 습도 `≤40 \|\| ≥80`을 warn 색으로, `lib/outdoor-index.ts`는 기온 ≥33·DI≥76을 `decisiveDeterrent`로 "좋음" 라벨 차단, 건강팁은 `폭염`을 정식 카테고리로. **히어로 상태 판정만 몰랐다.** 2026-07-26 결함("주의색 카드 + 근거 칩 0개")의 **역방향**이다. **결정:** ① 더위를 warn으로 — 기상청 불쾌지수 `DI≥80` 또는 기온 `≥33°` → "매우 심함", `DI≥76` → "심함". **폭염과 고온다습을 하나의 신호로 합쳐 센다**(outdoor-index가 heatPenalty·diPenalty 중 큰 값만 적용해 중복 감점을 막는 것과 같은 이유 — 칩 2개로 세면 issueCount가 부풀고 상한 2개인 칩 행이 더위로 다 찬다). ② 추위 `≤0°`도 함께 — 같은 튜플에서 차용했고, 빼면 겨울에 같은 결함을 그대로 남긴다. ③ 일교차 `≥8°C` warn 승격(2026-07-26이 임계까지 적어두고 보류한 항목). **임계는 전부 `outdoor-index.ts decisiveDeterrent`에서 차용해 새 숫자를 만들지 않았다** — warn 어휘가 다른 화면과 어긋난 것이 애초의 결함이므로. **고습을 습도 ≥80% 단독 warn으로 하지 않은 이유:** DI는 기온 24° 미만에서 계산되지 않아 서늘하고 습한 날(가을 비 온 뒤 습도 85%)을 더위로 오인하지 않는다 — 습도 단독 임계는 그 구분을 못 한다. ④ `EvidenceSlot.temp`를 **선택이 아닌 필수 필드**로 넣어 빠뜨림이 컴파일 오류가 되게 했다(선택으로 두면 신호가 조용히 사라져 같은 결함이 재발한다). ⑤ `slotNotables()`를 같은 커밋에서 함께 개정 — 안 하면 "히어로는 더위 경고 / 케어 플랜은 무난해요"로 갈린다. ⑥ `HeroDecisionBrief`에 처음부터 정의돼 있던 `heat`·`cold` 아이콘을 배선(더위→`heat`, 추위→`cold`) — 매핑이 없어 한 번도 쓰이지 않았다. 값은 등급 표기만 쓴다("더위 매우 심함" ⭕ / "더위 35°" ❌ — 수치는 기준값 블록의 몫). **칩 우선순위는 기존 6종을 건드리지 않고 뒤에 붙였다**(더위 6·추위 7·일교차 8) — 재정렬은 다른 이슈가 겹친 날의 칩 구성을 바꾸는 별건이고, AI가 더위를 1순위로 꼽은 날은 `ctxIssue` 승격이 이미 맨 앞으로 올린다. | 사용자 지적("칩 색상이 이상 지표면 강조로 나왔던 것 같은데 지금은 아무 강조도 없다") → 회귀 여부부터 확정(내 커밋 diff에 판정 로직 변경 0건, 더위·고습은 이력상 한 번도 warn이 아니었음) 후 승인 요청. 검증: 유닛 테스트 16건 추가(제보 재현값 32°·습도 70 → caution, 폭염 단독, DI 76~79, 서늘+고습 미발동, 결측, 중복 미계상, 한파, 일교차 8↑/7 경계) + 기존 양방향 불변식에 새 신호 3종 추가. 렌더 실측(Chromium 390px): pill 주황 tint + 온도계 아이콘, 칩 "더위 매우 심함", 케어 플랜 4슬롯이 "무난해요"→"더위 매우 심함". 변경 `lib/hero-brief.ts`·`app/(main)/home/page.tsx`·`lib/hero-brief.test.ts`·DESIGN.md |
