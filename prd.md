...
|------|------|
| **팀명** | 아이데이 (AiDay) |
| **작성일** | 2026-07-07 (v2 — 문제정의 프레임워크 v2 반영 재작성) |
...
| 이름 | 소속 | 역할 | 이메일 |
|------|------|------|--------|
| 이은상 | KAIST MBA | 1인 창업자 — 기획·검수·의사결정 총괄 | katherineeslee@gmail.com |
| Claude Code | AI 협업 도구 | 구현·코드 리뷰·QA 실행 ([섹션 12](#12-팀원별-역할-분담) 참조) | — |
...
> [!IMPORTANT]
> **기준 축 (절대 원칙):** 아이데이는 '외출 준비 도우미'가 아니다. 환경 해석에서 시작되는 **부모의 하루 첫 육아 판단 그 자체**를 지원한다. 외출·등원은 그 판단이 실행되는 대표적 장면일 뿐이며, **외출하지 않는 날에도 브리핑은 유효해야 한다** (실내 습도 관리·환기 시점·피부 케어라는 판단은 매일 발생한다).
...
부모의 하루 육아는 매일 아침 "오늘은 어떤 날인가, 그래서 내 아이에게 뭐가 필요한가"라는 판단에서 시작된다. 문제는 이 첫 판단이 출근·등원 준비가 겹치는 가장 바쁜 시간대에, 매일, 처음부터 다시 수행된다는 것 — 그리고 **노력해도 실패한다**는 것이다.

대표 시나리오([섹션 04](#04-사용자는-누구인가) 페르소나): 아침에 현재 기온 20°를 확인하고 합리적으로 면 티셔츠를 입혔지만, 산책 시간 28°에 땀에 젖고 하원 시간 17°에 추위에 떤다. 판단에 필요한 입력은 4개 축(시간대별 환경 궤적 / 아이 체질 / 아이의 오늘 일정 / 케어 옵션)인데, 바쁜 아침에는 가장 접근하기 쉬운 입력값(현재 날씨) 하나로 판단이 축소되기 때문이다. **실패의 원인은 부모의 노력 부족이 아니라 판단 구조의 결함이다.**
...
- 만 6세 이하 자녀를 둔 맞벌이 가구 **53.2%** (성평등가족부, 2025 — '15년 대비 15.1%p 증가)
- 국내 소아 아토피 유병률 최대 **20%** (환경부, 2023)
- 자체 FGI(만 1~8세 부모): **88.2%**가 "파편화된 정보를 아이 기준으로 해석하는 과정 자체"를 최대 부담으로, 옷차림 결정이 어려운 상황 1위는 **"일교차가 큰 환절기"(82.4%)** — 위 실패 시나리오와 정확히 일치
...
> [!NOTE]
> 대표 페르소나 "김서연"은 FGI 응답 패턴(일교차 어려움 82.4%, 해석 부담 88.2%)과 실사용 시나리오를 결합해 구성했으며, **판단 엔진의 표준 테스트 케이스**([섹션 05](#05-문제-해결을-위한-기능)·[11](#11-성과-측정-방법-kpi))로도 사용한다.
...
| **지불 의사** | FGI 사용 의향 89%, 최고 수요 기능 "당일 아침 자동 알림"(76.5%). 유료 전환 의사는 미검증 — 공개 베타는 무료 운영, 리텐션 검증 우선. 사용 빈도는 매일 아침 1회 이상 (비외출일 포함) |
...
### P0 — 선제 브리핑 발송: 당일 아침 웹 푸시 (PWA)

> [!NOTE]
> 프로세스 재정의의 핵심: "부모가 묻고 AI가 답한다" → **"AI가 먼저 제안하고 부모가 승인한다"**. 부모가 기상했을 때 판단이 이미 완료되어 도착해 있어야 한다.
> 발송 종류를 당일 아침으로 확정한 근거: FGI 수요 1위(당일 아침 자동 알림 76.5%) + 판단 입력(당일 시간대별 예보)의 신선도. 전날 밤 알림은 동일 인프라의 P1.

- **PWA 전환:** manifest + Service Worker ("홈 화면에 추가" 설치 경험)
- **웹 푸시 구독:** 알림 설정 화면([S-002](#화면-s-002-알림-설정-신설--마이페이지-하위))에서 권한 요청 → 구독 정보 DB 저장
- **당일 아침 브리핑 발송:** 온보딩 설정(등원 N분 전, 기본 30분 전 — 미설정 시 07:10)에 당일 예보 기반 브리핑 요약을 푸시 발송. Vercel Cron 10분 폴링 + 발송 이력 테이블로 중복 방지
...
| 같은 날 이미 발송된 사용자 | Cron 재실행 | 중복 발송되지 않는다 |
| 당일 예보 조회 실패 | 발송 시각 도래 | 해당 회차는 발송 스킵(잘못된 정보 발송 금지)되고 실패 사유가 기록된다 |
...
- 전날 밤 알림 (아침 브리핑과 동일 인프라 — 발송 종류 1개 추가)
- 카카오톡 "나에게 보내기" 알림 채널 (카카오 로그인 + `talk_message`)
...
- 카카오 알림톡(비즈메시지) — 승인 리드타임이 6주 초과, 공개 베타 검증 후 착수
...
| 상태 | 정의 |
|------|------|
| **동작** | [알림 허용하기] → 브라우저 권한 프롬프트 → 허용 시 푸시 구독 생성·DB 저장 → "알림 켜짐 ✓" 전환. 발송 기준 변경 → `children.notif` 즉시 갱신 |
| **빈 상태** | 권한 미요청 → 안내 문구 + [알림 허용하기] 버튼 |
| **에러 — 권한 거부** | "브라우저 설정에서 알림을 허용해주세요" + 브라우저별 설정 경로 안내 |
| **에러 — iOS Safari 미설치** | "홈 화면에 추가하면 알림을 받을 수 있어요" PWA 설치 안내 |
...
| **내용 규칙** | 제목은 브리핑 hook 형식(25자 이내), 본문은 당일 예보×일정 교차 기반 핵심 액션 1~2개. 건강 결과 약속 표현 금지(표현 안전 필터 적용) |
...
| **신규 도입** | web-push(VAPID) + Service Worker — 푸시 발송 / Vercel Cron — 발송 스케줄러(10분 폴링) / PostHog — 제품 계측 / 레이트리밋 카운터 — Supabase 테이블 기반(별도 인프라 추가 없음) |
...
| `notifications_log` | user_id, child_id, type(morning), scheduled_for, sent_at, status, fail_reason | 중복 발송 방지의 근거 테이블 |
...
| Phase 4 | 4~5주차 | 선제 브리핑 (PWA + 아침 웹 푸시) | S-002에서 구독 성공 / 등원 30분 전(또는 07:10) ±10분 내 푸시 도달 / OFF 사용자 미발송 / 중복 발송 없음 / 예보 실패 시 발송 스킵 |
| Phase 5 | 6주차 | PostHog 계측 + QA·공개 베타 | KPI 이벤트 대시보드 확인(수용/거부 로그 포함) / QA 스킬 전체 화면 점검 통과 / build·lint·표준 케이스 통과 / 공개 링크 배포 |
...
| 푸시 오픈율 (발송 대비 24시간 내 클릭) | ≥ 40% | 선제 브리핑이 아침 진입점으로 작동하는가 |
...
| 아침 푸시 정시 도달률 (±10분) | ≥ 95% | `notifications_log` 집계 |
