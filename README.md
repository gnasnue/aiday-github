...
| 오늘의 옷차림 추천 (OOTD) | ✅ 날씨 + 프로필 규칙 기반 (코디 미리보기 이미지는 더미) |
| 근거 기반 건강팁 | ✅ 정적 콘텐츠 — 홈 "오늘의 건강 팁" 시트 + `/tips` 전체 가이드 |
| 하루 = 성장 노트 (알림장 → 저녁 대화 거리 · 30일 성장) | ✅ Claude 연동 (2026-07-29) — 원문은 기기에만 7일 보관 |
| 오늘의 마무리 (저녁 결과 회수 → 다음 판단 반영) | ✅ 2스텝 세션 — 로컬 정본, 서버 영속화는 예정 |
| 이번 주 컨디션 예보 | ✅ 주간 예보 × 체질 × 저녁 기록 |
| 환절기 케어 패스 (월 990원) | 🔧 사전예약만 — 결제 미연동 |
| 회원가입 / 로그인 | ✅ 이메일 + Google OAuth, 비밀번호 재설정 (Supabase) |

> 화면별 상세 구현 현황과 더미 데이터 잔여분은 [SPEC.md](./SPEC.md)의 "페이지 구현 현황 요약" 참조.

라이브: [myaiday.co](https://myaiday.co) (Vercel 배포)
...
**하루 루프 쐐기 (2026-07-28~29 착수, 원안 밖):** 아침 판단(홈)에서 끝나지 않고 기관에 전달하고
저녁에 회수하는 루프를 최소 구현으로 얹었다 — 성장 노트(알림장 → 저녁 대화 거리 · 30일 성장),
오늘의 마무리, 이번 주 컨디션 예보, 환절기 케어 패스 사전예약. **기능 완성이 아니라 행동률·지불
의향 측정용**이며 게이트 미달 시 제거한다 ([docs/PRODUCT-DECISIONS.md](./docs/PRODUCT-DECISIONS.md) §3-7·§3-8).
...
| `GET /api/weather/weekly?region=서울&lat=&lon=` | 주간 예보 병합 (D0~D2 단기 + D+3~ 중기) — 이번 주 컨디션 예보용 |
| `POST /api/report` | Claude AI 육아 판단 리포트 생성 (레이트리밋: 로그인 20 / 게스트 10회·일) |
| `POST /api/noteboard` | 알림장 → 저녁 대화 거리 생성 (로그인 필수 · 하루 5회 · 원문 서버 미저장) |
| `POST /api/feedback` | 리포트 👍/👎 · 자유 의견 · 케어 패스 사전예약 |
