...
// 꽃가루농도위험지수 단계 매핑 — 낮음=0 · 보통=1 · 높음=2 · 매우높음=3.
// 근거: 공공데이터포털 "기상청_꽃가루농도위험지수 조회서비스(3.0)" 첨부 설명서의
// "단계 및 범위" 표(참나무·소나무·잡초류 3종 모두 동일). 상한은 3이고 4는 없다.
// 상한을 4로 잡으면 "매우높음"이 영원히 안 나오고 전 단계가 한 칸씩 낮게 표시되므로
// (알레르기 체질 아이에게 위험을 과소 표기하는 방향) 이 매핑은 함부로 바꾸지 않는다.
// 범위 밖(4 이상) 값이 와도 최고 단계로 떨어지게 부등호 비교를 유지한다.
export const pollenLevelOf = (g: number): PollenLevel =>
  g >= 3 ? "매우높음" : g >= 2 ? "높음" : g >= 1 ? "보통" : "낮음";

// 지수 → 라벨 (홈 상단 환경 칩·시간대 카드 공용).
// null(미측정·제공 기간 외)은 렌더 폴백으로 "낮음".
export const pollenLabel = (g: number | null): PollenLevel =>
  g == null ? "낮음" : pollenLevelOf(g);
