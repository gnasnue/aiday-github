...
export const SCENARIOS = [
  ...
  // 드러나는지를 기계 판정한다 — 표현 자체는 자유, 키워드는 판단 유형의 흔적만 잡는다.
  {
    ...
    expects: [
      "우산은 등원길",
      "마스크는 하원·오후 대비로 시점 교차",
      "그친 뒤 재비산 메커니즘",
      "꽃가루는 일 단위 등급만 입력되므로 재비산 시점은 단정이 아니라 가능성 표현",
    ],
...
export const runChecks = (s, r) => {
  ...
  // 출력으로 복사되던 것. 정적 린트(lib/prompts/report.test.ts)와 같은 목록을 본다.
  const metaComp = body.match(/[^\n]{0,14}(자체보다|자체가 아니라|보다 (더 )?중요)[^\n]{0,14}/);
  add("메타 비교 구문 없음(자체보다·보다 중요)", !metaComp, metaComp ? `"${metaComp[0].trim()}"` : "");
  ...
  // ── 시점 단정 (2026-07-27) ─────────────────────────────────────
  // 꽃가루는 "오늘 최고" 등급 하나, 대기질은 현재값만 프롬프트에 들어간다(route.ts
  // pollenSummary·airSummary — 기상청 꽃가루 API가 일 단위, 에어코리아가 실시간 측정값).
  // 즉 이 지표들에는 "시간에 따라 이렇게 변한다"는 입력이 애초에 없다 — "비 그친 뒤 낮
  // 동안 마르면서 다시 날려요"처럼 변화를 사실로 말하면 데이터가 뒷받침하지 않는 단정이다.
  // 메커니즘 추론 자체는 프롬프트가 허용한 제품 가정이므로 가능성 보존형("다시 날리기
  // 쉬워요")이면 통과시키고 단정형만 잡는다. 기온·강수·자외선은 시간대별 입력이 있어
  // 대상에서 제외 (Codex eng-review 2026-07-27 지적). 검사식은 docs/report-eval 누적
  // 산출물 358건에 소급 적용해 교정했다 — 적발 6건 전부 v25 E-AHA-1의 실제 단정, 오탐 0.
  // 가능성 보존 표현. "-기 쉽다"는 활용형이 많으므로(쉬워요·쉬워서·쉬워,·쉬운데·쉽고)
  // 어미를 열거하지 않고 구문 자체("…기 쉬/쉽")로 잡는다 — 어미 열거는 실제로 새로 나온
  // 연결형 "다시 날리기 쉬워,"를 오탐으로 잡았다. "날리기 시작"류 단정은 걸리지 않는다.
  const HEDGE = /기 쉬|기 쉽|수 있|수도 있|가능성|편이에요|편이라|모르니|혹시/;
  const NO_HOURLY = "꽃가루|미세먼지|초미세먼지|통합대기|대기질";
  const CHANGE_ASSERT = "다시 날[리려]|날리기 시작|재비산|다시 퍼[지져]|짙어[지져]|나빠[지져]|심해[지져]|다시 올라";
  // 지표→변화 서술이 같은 절 안에 40자 이내로 붙어 있을 때만 잡는다. 절(— 구분) 단위로
  // 봐야 뒷절의 무관한 완화 표현("챙기기 쉬워요")이 앞절의 단정을 가려주지 않는다.
  const assertRe = new RegExp(`(?:${NO_HOURLY})[^—–]{0,40}?(?:${CHANGE_ASSERT})`);
  const timeAssert = msgLines
    .flatMap((l) => (new RegExp(NO_HOURLY).test(l) ? l.split(/\s*[—–]\s*/) : []))
    .find((c) => assertRe.test(c) && !HEDGE.test(c));
  add("시점 단정 없음(일 단위·현재값 지표)", !timeAssert, timeAssert?.slice(0, 80) ?? "");
  ...
  // "여벌 옷"과 "여벌 상의"는 **별개 항목**이다 — 표기 변형이 아니라 챙길 양이 다른 처방이라
  // 한 그룹으로 묶으면 hook은 상의, checklist는 옷인 갈림이 통과해버린다 (2026-07-27).
  const ITEM_ALIASES = {
    우산: ["우산"],
    우비: ["우비"],
    마스크: ["마스크"],
