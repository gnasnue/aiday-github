...
| 경로 | 역할 |
|------|------|
| `app/(main)/*` | 탭 5종: home(AI 리포트·저녁 회수 진입)·env·outfit·day(**{아이}의 성장 노트** — 오늘 알림장 / 30일 성장)·me. 탭 밖: tips(전체 가이드)·pass(케어 패스) |
...
- **docs/PRODUCT-DECISIONS.md** — 성공 지표·출시 기준·확정된 제품 결정. 출시 범위나 보류 항목 판단 시 참조 (하루 루프·케어 패스의 존치 게이트는 §3-7·§3-8).
- **PRD.md** — 6주 고도화 스코프·P0·KPI. 구현 현황 스냅샷(§05)이 코드와 문서의 차이를 기록한다.
...
프로젝트 스킬은 `.agents/skills/`에 있다. 요청이 아래 스킬에 해당하면 Skill 도구로 호출한다. When in doubt, invoke the skill.
